...
class Peao(Base):
    ...
    def movimentos_possíveis(self, info_peças):
        ...
        direcao = 1 if cor_peao == 'branca' or cor_peao == 'branco' else -1
        
        # Movimento para frente (1 casa)
        nova_linha = linha_atual + direcao
        if 0 <= nova_linha <= 7 and (nova_linha, coluna_atual) not in casas_ocupadas:
            ...
            linha_inicial = 1 if cor_peao == 'branca' or cor_peao == 'branco' else 6
            if linha_atual == linha_inicial:
                nova_linha = linha_atual + 2 * direcao
                if 0 <= nova_linha <= 7 and (nova_linha, coluna_atual) not in casas_ocupadas:
                    movimentos_possíveis.append((nova_linha, coluna_atual))
        ...
        for d_coluna in [-1, 1]:
            ...
            if 0 <= nova_linha <= 7 and 0 <= nova_coluna <= 7:
                if (nova_linha, nova_coluna) in casas_ocupadas:
                    ...
                    if (cor_peao == 'branca' or cor_peao == 'branco') and (peça_alvo.cor == 'preta' or peça_alvo.cor == 'preto'):
                        movimentos_possíveis.append((nova_linha, nova_coluna))
                    elif (cor_peao == 'preta' or cor_peao == 'preto') and (peça_alvo.cor == 'branca' or peça_alvo.cor == 'branco'):
                        movimentos_possíveis.append((nova_linha, nova_coluna))
